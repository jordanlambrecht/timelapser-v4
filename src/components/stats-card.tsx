      ...
      <CardContent className='p-6 relative'>
        <div className='flex items-start justify-between'>
          <div className='space-y-2 flex-1'>
            ...
            {description && (
              <p className='text-grey-light/60 text-sm'>{description}</p>
            )}
            {trend && (
                ...
                <span
                  className={cn(
                    ...
                    trend.value > 0
                      ...
                      : trend.value < 0
                      ? "text-failure bg-failure/20"
                      : "text-grey-light/60 bg-grey-light/10"
                  )}
                >
                  {trend.value > 0 ? "↗" : trend.value < 0 ? "↘" : "→"}{" "}
                  {Math.abs(trend.value)}%
                </span>
                <span className='text-grey-light/60 text-xs'>
                  {trend.label}
                </span>
              </div>
            )}
