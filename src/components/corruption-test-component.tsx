...
export default function CorruptionTestComponent() {
  const [testResult, setTestResult] = useState<CorruptionTestResult | null>(
    null
  )
  ...
  useEffect(() => {
    if (files.length > 0 && files[0]?.file && !isAnalyzing) {
      ...
      // Check if it's a File object (not FileMetadata)
    ...
    <Card>
      <CardHeader>
        ...
        <p className='text-sm text-muted-foreground'>
          Upload test images to see how the corruption detection system analyzes
          them
        </p>
      </CardHeader>
      <CardContent>
                ...
                {previewUrl ? (
                  ...
                  <div className='flex flex-col items-center justify-center px-4 py-3 text-center'>
                    ...
                    <p className='text-muted-foreground text-xs'>
                      Max size: {maxSizeMB}MB • Images are analyzed and
                      discarded
                    </p>
                  </div>
                )}
                  ...
                  <div className='border rounded-lg p-3'>
                    <div className='flex items-center justify-between mb-2'>
                      ...
                      <Badge variant='outline'>
                        {testResult.fast_score}/100
                      </Badge>
                    </div>
                    <div className='space-y-1'>
                      {Object.entries(
                        testResult.detection_details.fast_detection
                      ).map(([key, result]) => (
                        <div
                          key={key}
                    ...
                    <div className='border rounded-lg p-3 bg-red-50 border-red-200'>
                      ...
                      <div className='space-y-1'>
                        {testResult.failed_checks.map((check, index) => (
                          <div
                            key={index}
                            className='flex items-center space-x-2 text-xs text-red-700'
                          >
