...
import { useOverlayPreview, type OverlayPreviewRequest } from "@/hooks/use-overlay-preview"
...
export function OverlayPreviewModal({
  ...
  const { previewData, isGenerating, error, generatePreview, clearPreview } = useOverlayPreview()
  ...
  const handleDownloadPreview = () => {
    if (previewData?.image_path) {
      ...
      link.download = `overlay-preview-${presetName.toLowerCase().replace(/\s+/g, '-')}.png`
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <Button 
                    ...
                    disabled={!selectedCameraId || isGenerating || (selectedCameraId && previewGenerationInProgress.has(selectedCameraId))}
                    className="bg-gradient-to-r from-purple to-cyan hover:from-purple/90 hover:to-cyan/90"
                  >
                    {(isGenerating || (selectedCameraId && previewGenerationInProgress.has(selectedCameraId))) ? (
                      <>
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <Card className="bg-gray-900/30 border-gray-700">
                  <CardContent className="p-4">
                    <h4 className="text-sm font-medium mb-3 text-gray-300">Original Image</h4>
                ...
                <Card className="bg-gray-900/30 border-gray-700">
                  <CardContent className="p-4">
                    <h4 className="text-sm font-medium mb-3 text-green-400">With Overlay</h4>
                    <div className="relative bg-gray-800 rounded-lg overflow-hidden">
                      <img
                        ...
                        className="w-full h-auto"
                        onError={(e) => {
                          e.currentTarget.src = "/assets/placeholder-overlay.jpg"
                        }}
                      />
                    </div>
                    <p className="text-xs text-muted-foreground mt-2">
              ...
              <Card className="bg-gray-900/30 border-gray-700">
                <CardContent className="p-4">
                  <h4 className="text-sm font-medium mb-3">Overlay Configuration</h4>
                  <div className="grid md:grid-cols-2 gap-4 text-xs">
                    <div>
                      <h5 className="font-medium text-gray-300 mb-2">Global Options</h5>
                      <div className="space-y-1 text-muted-foreground">
                        <div>Opacity: {overlayConfig.globalOptions.opacity}%</div>
                        <div>Font: {overlayConfig.globalOptions.font}</div>
                        <div>Margins: {overlayConfig.globalOptions.xMargin}px × {overlayConfig.globalOptions.yMargin}px</div>
                        {overlayConfig.globalOptions.dropShadow && (
                          <div>Drop Shadow: {overlayConfig.globalOptions.dropShadow}px</div>
                        )}
                      </div>
                    </div>
                    <div>
                      <h5 className="font-medium text-gray-300 mb-2">Overlay Elements</h5>
                      <div className="space-y-1 text-muted-foreground">
                        {Object.entries(overlayConfig.overlayPositions).map(([position, config]: [string, any]) => (
                          <div key={position}>
                            {position}: {config.type} ({config.textSize}px)
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {!previewData && !isGenerating && !error && (
            <Alert className="border-cyan/30 bg-cyan/10">
              ...
              <AlertDescription className="text-sm text-cyan">
                Select a camera and click "Generate Preview" to see how your overlay configuration will look on real camera images.
              </AlertDescription>
            </Alert>
          )}
        </div>
