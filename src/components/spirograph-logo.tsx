...
export function SpirographLogo({ className, size = 48 }: SpirographLogoProps) {
  ...
  const generateSpirographPath = (R: number, r: number, d: number, steps: number = 500) => {
    const path: string[] = []
    const centerX = 50
    const centerY = 50
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      const x = centerX + ((R - r) * Math.cos(t) + d * Math.cos((R - r) / r * t)) * 0.8
      const y = centerY + ((R - r) * Math.sin(t) - d * Math.sin((R - r) / r * t)) * 0.8
  ...
  const spirographs = [
    { R: 30, r: 7, d: 15, color: 'var(--color-pink)', opacity: 0.8, duration: '20s' },
    { R: 25, r: 5, d: 12, color: 'var(--color-cyan)', opacity: 0.7, duration: '15s' },
    { R: 20, r: 8, d: 10, color: 'var(--color-purple)', opacity: 0.6, duration: '25s' },
    { R: 35, r: 12, d: 8, color: 'var(--color-purple-light)', opacity: 0.5, duration: '30s' },
  ]
    ...
    <div className={cn("relative", className)} style={{ width: size, height: size }}>
      ...
      >
        <defs>
          ...
          <linearGradient id="spirograph-gradient-1" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <linearGradient id="spirograph-gradient-2" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <g className="animate-spin" style={{ animationDuration: '60s', transformOrigin: '50px 50px' }}>
          ...
          {spirographs.map((config, index) => (
            <g key={index} className="animate-spin" style={{ 
              ...
              <path
                ...
                fill="none"
                stroke={index < 2 ? `url(#spirograph-gradient-${index + 1})` : config.color}
                strokeWidth="0.5"
