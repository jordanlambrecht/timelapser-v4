...
import type { OverlayPreset, OverlayItem } from "@/hooks/use-overlay-presets"
...
export function PresetListItem({
  ...
  // Extract positions from overlay config (unified format)
  const positions =
    preset.overlay_config?.overlayItems?.map(
      (item: OverlayItem) => item.position
    ) || []
