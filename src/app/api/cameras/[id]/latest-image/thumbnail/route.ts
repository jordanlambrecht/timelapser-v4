...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${fastApiUrl}/api/cameras/${cameraId}/latest-image/thumbnail`,
      {
        method: "GET",
    ...
    console.error("Latest image thumbnail proxy error:", error)
    return NextResponse.json(
      { error: "Failed to serve latest image thumbnail" },
      { status: 500 }
    )
  }
}
