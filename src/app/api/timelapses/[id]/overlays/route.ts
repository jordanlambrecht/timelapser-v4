...
export async function GET(
  ...
  try {
    const timelapseId = params.id
    console.log(`Fetching overlay configuration for timelapse ${timelapseId}`)
    
    const response = await fetch(`${FASTAPI_URL}/api/overlays/config/${timelapseId}`, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`Backend overlay config endpoint returned ${response.status}`)
    ...
    console.log(`Successfully fetched overlay configuration for timelapse ${timelapseId}`)
...
export async function POST(
  ...
  try {
    ...
    const data = await response.json()
    console.log(`Successfully created overlay configuration for timelapse ${timelapseId}`)
...
export async function PUT(
  ...
  try {
    ...
    const data = await response.json()
    console.log(`Successfully updated overlay configuration for timelapse ${timelapseId}`)
    return NextResponse.json(data)
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    console.log(`Successfully deleted overlay configuration for timelapse ${timelapseId}`)
    return NextResponse.json({ message: "Overlay configuration deleted successfully" })
  } catch (error) {
