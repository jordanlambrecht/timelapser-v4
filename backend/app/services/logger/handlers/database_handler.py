...
class LegacyLoguraDatabaseHandler:
    """
    ...
    def __call__(self, record) -> None:
        ...
        try:
            ...
            print(f"Record info: level={getattr(record, 'level', 'unknown')}, name={getattr(record, 'name', 'unknown')}")
            self._healthy = False
    ...
    def _determine_source(self, logger_name: str, extra_data: Dict[str, Any]) -> str:
        ...
        if "worker" in logger_lower:
            return "worker"
        elif "api" in logger_lower or "request" in logger_lower:
            return "api"
        elif "camera" in logger_lower:
            return "camera"
        elif "system" in logger_lower:
            return "system"
