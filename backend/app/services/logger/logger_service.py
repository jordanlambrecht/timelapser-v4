...
from ...enums import LogLevel, LogSource, LoggerName, LogEmoji, SSEPriority
...
from ...utils.time_utils import get_timezone_aware_timestamp_async
...
from .services.broadcast_service import SSEBroadcastService
...
class LoggerService:
    ...
    def _initialize_handlers(self) -> None:
        ...
        # Database handler (always enabled for storage)
        ...
        if self.enable_sse_broadcasting:
            self.sse_broadcast_service = SSEBroadcastService(
                self.sse_ops, 
                self.sync_sse_ops
            )
    ...
    async def log_error(
        ...
        Args:
            message: Error message (should include emoji for consistency)
        ...
        await self._log_entry(
            ...
            sse_priority=SSEPriority.HIGH if level in [LogLevel.ERROR, LogLevel.CRITICAL] else SSEPriority.NORMAL
        )
    ...
    async def log_worker(
        ...
        Args:
            message: Worker message (should include emoji for consistency)
    ...
    async def log_system(
        ...
        Args:
            message: System message (should include emoji for consistency)
    ...
    async def log_capture(
        ...
        Args:
            message: Capture message (should include emoji for consistency)
    ...
    async def _log_entry(
        ...
        try:
            ...
            enriched_context = self.context_extractor.extract_context(
                message, extra_context, correlation_id
            )
            ...
            if broadcast_sse and self.enable_sse_broadcasting:
                await self.sse_broadcast_service.broadcast_log_event(
    ...
    async def cleanup_old_logs(self, days_to_keep: int = 30) -> int:
        ...
        """
        return await self.cleanup_service.cleanup_old_logs(days_to_keep)
    ...
    def get_handler_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "console_handler": {
                ...
                "healthy": self.console_handler.is_healthy() if self.enable_console else None
            },
            "file_handler": {
                "enabled": self.enable_file_logging,
                "healthy": self.file_handler.is_healthy() if self.enable_file_logging else None
            },
            "sse_broadcast_service": {
                "enabled": self.enable_sse_broadcasting,
                "healthy": self.sse_broadcast_service.is_healthy() if self.enable_sse_broadcasting else None
            }
        }
