...
class ThumbnailVerificationService:
    ...
    async def verify_single_image(
        self, image_id: int, check_both_structures: bool = True
        ...
        try:
            ...
            return ThumbnailVerificationResult(
                image_id=image_id, camera_id=0, error=str(e), verified_at=utc_now()
            )
                        ...
                        if result.error:
                            ...
                            if result.thumbnail_exists:
                                ...
                                if result.thumbnail_size_bytes:
                                    summary.total_thumbnail_size_mb += (
                                        result.thumbnail_size_bytes / (1024 * 1024)
                                    )
                            else:
                            ...
                            if result.small_exists:
                                ...
                                if result.small_size_bytes:
                                    summary.total_small_size_mb += (
                                        result.small_size_bytes / (1024 * 1024)
                                    )
                            else:
    ...
    async def repair_missing_thumbnails(
        self, repair_request: ThumbnailRepairRequest
    ) -> ThumbnailRepairResult:
        """
        Queue repair jobs for missing thumbnails.
        ...
        try:
            ...
            if repair_request.image_ids:
                target_images = await self.image_ops.get_images_by_ids(
                    repair_request.image_ids
                )
            else:
                ...
                try:
                    ...
                    if job:
                        ...
                        result.errors.append(
                            f"Failed to queue repair job for image {image.id}"
                        )

                except Exception as e:
                    result.errors.append(
                        f"Error queuing repair job for image {image.id}: {str(e)}"
                    )

            result.repair_jobs_queued = jobs_queued
            result.success = True
            result.message = (
                f"Queued {jobs_queued} repair jobs for {len(target_images)} images"
            )
