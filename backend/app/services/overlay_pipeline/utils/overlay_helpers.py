...
class OverlaySettingsResolver:
    ...
    def _merge_configurations(
        base: OverlayConfiguration,
        override: OverlayConfiguration
    ) -> OverlayConfiguration:
        """
        ...
        merged_global_options = GlobalOverlayOptions(
            opacity=override.globalOptions.opacity if override.globalOptions.opacity != base.globalOptions.opacity else base.globalOptions.opacity,
            font=override.globalOptions.font if override.globalOptions.font != base.globalOptions.font else base.globalOptions.font,
            xMargin=override.globalOptions.xMargin if override.globalOptions.xMargin != base.globalOptions.xMargin else base.globalOptions.xMargin,
            yMargin=override.globalOptions.yMargin if override.globalOptions.yMargin != base.globalOptions.yMargin else base.globalOptions.yMargin,
        )
    ...
    @staticmethod
    def validate_configuration_completeness(config: OverlayConfiguration) -> Dict[str, Any]:
        """
        ...
        global_issues = OverlaySettingsResolver._validate_global_options(config.globalOptions)
...
class OverlayPresetManager:
    ...
    def extract_preset_summary(preset: OverlayPreset) -> Dict[str, Any]:
        ...
        for position, overlay_item in preset.overlay_config.overlayPositions.items():
            if overlay_item.type in ['date', 'date_time', 'time', 'frame_number', 'day_number', 'custom_text', 'timelapse_name']:
                text_overlays.append(overlay_item.type)
            elif overlay_item.type in ['temperature', 'weather_conditions', 'weather_temp_conditions']:
