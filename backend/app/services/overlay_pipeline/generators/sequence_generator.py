...
class SequenceGenerator(BaseOverlayGenerator):
    ...
    def generate_content(
        ...
        try:
            ...
            logger.debug(f"🔍 Sequence overlay validation passed for type: {overlay_item.type}")
            ...
            logger.error(f"❌ Validation error in sequence overlay generation: {e}")
            raise
        except Exception as e:
            logger.error(f"❌ Failed to generate sequence overlay content: {e}")
    ...
    def _generate_frame_number(
        ...
        logger.debug(f"✅ Frame number overlay generated successfully")
        return result
    ...
    def _generate_day_number(
        ...
        logger.debug(f"✅ Day number overlay generated successfully")
        return result
    ...
    def _calculate_day_number_from_dates(
        ...
        Returns:
            Day number (1-based)
        """
        logger.debug(f"🔢 Calculating day number from dates: start={start_date.date()}, current={current_date.date()}")
        ...
        logger.debug(f"🔢 Day number calculated: {day_number} (delta: {delta.days} days)")
        return day_number
    ...
    def validate_overlay_item(self, overlay_item: OverlayItem) -> None:
        ...
        """
        logger.debug(f"🔍 Validating sequence overlay item of type: {overlay_item.type}")
        ...
        try:
            ...
            validated_leading_zeros = validate_boolean_property(overlay_item.leadingZeros, "leadingZeros")
            validated_hide_prefix = validate_boolean_property(overlay_item.hidePrefix, "hidePrefix")
            
            if validated_leading_zeros is not None:
                logger.debug(f"🔢 Validated leadingZeros: {validated_leading_zeros}")
            if validated_hide_prefix is not None:
                logger.debug(f"🔢 Validated hidePrefix: {validated_hide_prefix}")
                
        except ValueError as e:
            logger.error(f"❌ Sequence validation failed: {e}")
            raise
        
        logger.debug(f"✅ Sequence overlay validation completed for type: {overlay_item.type}")
        if overlay_item.leadingZeros:
