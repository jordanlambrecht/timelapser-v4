...
class DateTimeGenerator(BaseOverlayGenerator):
    ...
    def generate_content(
        ...
        try:
            ...
            logger.debug(f"🔍 Overlay item validation passed for type: {overlay_item.type}")
            
            # Convert timestamp to database timezone using existing utility
            timestamp = context.image_timestamp
            logger.debug(f"⏰ Converting timestamp to database timezone")
            ...
            if context.settings_service:
                ...
                logger.warning(f"⚠️ No settings service provided, using timestamp as-is: {timestamp}")
            ...
            logger.debug(f"✅ Successfully generated {overlay_item.type} overlay content: '{result}'")
            return result
                
        except ValueError as e:
            logger.error(f"❌ Validation error in datetime overlay generation: {e}")
            raise
        except Exception as e:
            logger.error(f"❌ Failed to generate date/time overlay content: {e}")
    ...
    def _convert_frontend_format(self, frontend_format: str) -> str:
        ...
        """
        logger.debug(f"📐 Converting frontend format '{frontend_format}' to Python strftime")
