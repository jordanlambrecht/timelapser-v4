...
class StaticOverlayTemplate:
    ...
    static_overlay_positions: Set[GridPosition]
...
class StaticOverlayTemplateCache:
    ...
    def __init__(self, max_cache_size_mb: int = 100, max_template_age_hours: int = 24):
        ...
        logger.info(f"StaticOverlayTemplateCache initialized: max_size={max_cache_size_mb}MB, max_age={max_template_age_hours}h")
    ...
    def create_template(
        ...
        with self._lock:
            ...
            logger.debug(f"Created template {cache_key}, cache size: {len(self._cache)} templates")
            return template
    ...
    def _generate_cache_key(
        ...
        global_config = {
            ...
            'backgroundColor': getattr(config.globalOptions, 'backgroundColor', '#000000'),
    ...
    def _identify_static_positions(self, config: OverlayConfiguration) -> Set[GridPosition]:
    ...
    def _generate_static_template(
        ...
        base_image_dimensions: Tuple[int, int],
        context: OverlayGenerationContext,
        static_positions: Set[GridPosition]
    ) -> Image.Image:
        """
        ...
        # Import OverlayRenderer here to avoid circular imports
        from ..utils.overlay_utils import OverlayRenderer
        
        # Create temporary config with only static overlays
        static_config_positions = {
            pos: item for pos, item in config.overlayPositions.items()
        ...
        static_config = OverlayConfiguration(
            overlayPositions=static_config_positions,
        ...
        context_data = {
            ...
            "timelapse_name": context.timelapse.name if context.timelapse else "Timelapse",
        ...
        try:
            ...
            async def render_template():
                return await renderer.render_overlay_async(template_image, context_data)
    ...
    def _cleanup_if_needed(self) -> None:
        ...
        current_size = sum(template.memory_size_bytes for template in self._cache.values())
        
        if current_size <= self.max_cache_size_bytes:
            return
        
        logger.info(f"Cache cleanup needed: {current_size} bytes > {self.max_cache_size_bytes} bytes")
        ...
        logger.info(f"Removed {removed_count} templates from cache, new size: {current_size} bytes")
