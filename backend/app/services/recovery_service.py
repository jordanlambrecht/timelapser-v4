...
class RecoveryService:
    ...
    async def perform_startup_recovery(
        ...
        logger.info("🔄 Starting startup recovery for all job types...")
        ...
        try:
            ...
            if log_recovery_details:
                logger.info("📸 Recovering stuck thumbnail generation jobs...")
            ...
            if log_recovery_details:
                logger.info("🎬 Recovering stuck video generation jobs...")
            ...
            if log_recovery_details:
                logger.info("🎨 Recovering stuck overlay generation jobs...")
            ...
            if total_failed > 0:
                logger.warning(
                    f"⚠️ {total_failed} jobs failed to recover during startup"
                )
...
class SyncRecoveryService:
    ...
    def perform_startup_recovery(
        ...
        """
        recovery_start_time = utc_now()
        logger.info("🔄 Starting startup recovery for all job types (sync)...")
        ...
        try:
            ...
            if log_recovery_details:
                logger.info("🧹 Performing startup file cleanup...")
