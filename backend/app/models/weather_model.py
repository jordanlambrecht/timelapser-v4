...
class WeatherSettings(BaseModel):
    ...
    sunrise_sunset_enabled: bool = Field(
        False, description="Enable sunrise/sunset time windows globally"
    )
...
class TimeWindowMode(BaseModel):
    ...
    time_start: Optional[str] = Field(
        None, description="Start time for 'time' mode (HH:MM:SS)"
    )
    time_end: Optional[str] = Field(
        None, description="End time for 'time' mode (HH:MM:SS)"
    )
    sunrise_offset: Optional[int] = Field(
        None, description="Sunrise offset in minutes for 'sun' mode"
    )
    sunset_offset: Optional[int] = Field(
        None, description="Sunset offset in minutes for 'sun' mode"
    )
