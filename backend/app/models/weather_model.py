...
class WeatherDataRecord(BaseModel):
    ...
    current_weather_icon: Optional[str] = Field(None, max_length=50, description="OpenWeather icon code")
    current_weather_description: Optional[str] = Field(None, max_length=255, description="Weather description")
    ...
    error_response_code: Optional[int] = Field(None, description="HTTP error code if failed")
...
class WeatherConfiguration(BaseModel):
    ...
    sunrise_sunset_enabled: bool = Field(False, description="Enable sunrise/sunset time windows")
...
class WeatherRefreshResult(BaseModel):
    ...
    weather_data: Optional[OpenWeatherApiData] = Field(None, description="Fetched weather data")
...
class WeatherSystemStatus(BaseModel):
    ...
    last_successful_fetch: Optional[datetime] = Field(None, description="Last successful data fetch")
...
class LegacyWeatherRefreshResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether refresh was successful")
    message: str = Field(..., description="Refresh result message")
    weather_data: Optional[LegacyWeatherData] = Field(None, description="Updated weather data")