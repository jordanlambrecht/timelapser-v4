...
class WorkflowStepResult(BaseModel):
    ...
    data: Dict[str, Any] = Field(default_factory=dict, description="Step-specific result data")
...
class CapturePipelineContext(BaseModel):
    ...
    initiated_by: str = Field(..., description="Source that initiated the capture (scheduler, manual, etc.)")
    initiated_at: datetime = Field(default_factory=datetime.utcnow, description="Workflow initiation timestamp")
...
class RTSPCaptureResult(BaseModel):
    ...
    file_size_bytes: Optional[int] = Field(None, description="Size of captured file in bytes")
    capture_timestamp: Optional[datetime] = Field(None, description="Actual capture timestamp")
...
class CorruptionAssessmentResult(BaseModel):
    ...
    fast_detection_score: Optional[float] = Field(None, description="Fast detection score")
    heavy_detection_score: Optional[float] = Field(None, description="Heavy detection score if enabled")
    is_acceptable: bool = Field(..., description="Whether image quality is acceptable")
    assessment_method: str = Field(..., description="Method used for assessment (fast_only, combined)")
    detection_duration_ms: float = Field(..., description="Time taken for corruption detection")
...
class ImageRecordResult(BaseModel):
    ...
    record_created: bool = Field(..., description="Whether database record was created successfully")
    weather_data_added: bool = Field(default=False, description="Whether weather data was included")
    metadata_enriched: bool = Field(default=False, description="Whether additional metadata was added")
    timelapse_updated: bool = Field(default=False, description="Whether timelapse counts were updated")
...
class OverlayGenerationResult(BaseModel):
    ...
    overlay_path: Optional[str] = Field(None, description="Path to generated overlay file")
    overlay_status: str = Field(..., description="Status of overlay (generated, disabled, fallback)")
    generation_duration_ms: Optional[float] = Field(None, description="Time taken for overlay generation")
    fallback_used: bool = Field(default=False, description="Whether fallback overlay was used")


class JobCoordinationResult(BaseModel):
    """Result from background job coordination."""
    thumbnail_job_queued: bool = Field(default=False, description="Whether thumbnail job was queued")
    video_automation_triggered: bool = Field(default=False, description="Whether video automation was triggered")
    jobs_queued_count: int = Field(default=0, description="Total number of jobs queued")
    job_queue_errors: List[str] = Field(default_factory=list, description="Any errors during job queueing")


class CapturePipelineResult(BaseModel):
    """Complete result from capture pipeline workflow execution."""
    overall_result: CaptureWorkflowResult = Field(..., description="Overall workflow result")
    context: CapturePipelineContext = Field(..., description="Original workflow context")
    step_results: List[WorkflowStepResult] = Field(default_factory=list, description="Results from individual steps")
    
    # Aggregated results from key steps
    rtsp_result: Optional[RTSPCaptureResult] = Field(None, description="RTSP capture result")
    corruption_result: Optional[CorruptionAssessmentResult] = Field(None, description="Corruption assessment result")
    image_result: Optional[ImageRecordResult] = Field(None, description="Image record creation result")
    overlay_result: Optional[OverlayGenerationResult] = Field(None, description="Overlay generation result")
    job_result: Optional[JobCoordinationResult] = Field(None, description="Job coordination result")
    
    # Workflow metadata
    total_duration_seconds: Optional[float] = Field(None, description="Total workflow execution time")
