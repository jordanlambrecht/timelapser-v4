...
class DatabaseHealth(BaseHealthModel):
    ...
    async_latency_ms: Optional[float] = Field(
        None, description="Async connection latency"
    )
    sync_latency_ms: Optional[float] = Field(
        None, description="Sync connection latency"
    )
    pool_status: Optional[Dict[str, Any]] = Field(
        None, description="Connection pool status"
    )
    error: Optional[str] = Field(None, description="Error message if unhealthy")
...
class SystemMetrics(BaseHealthModel):
    ...
    load_average: Optional[Dict[str, float]] = Field(
