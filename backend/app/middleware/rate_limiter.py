...
class SlidingWindowRateLimiter:
    ...
    async def cleanup_old_entries(self, max_age_seconds: int = 3600) -> int:
        ...
        async with self._lock:
            ...
            if cleaned_count > 0:
                logger.debug(f"🧹 Rate limiter cleaned up {cleaned_count} old client entries")
...
class RateLimitConfig:
    ...
    def get_limit_for_endpoint(cls, endpoint_pattern: str) -> Dict[str, int]:
        ...
        # Image serving endpoints
        if "/images/" in endpoint_pattern and any(x in endpoint_pattern for x in ["serve", "download"]):
