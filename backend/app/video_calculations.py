...
def calculate_video_fps(total_images: int, settings: VideoGenerationSettings) -> VideoCalculationResult:
    """
    ...
    if settings.video_generation_mode == VideoGenerationMode.STANDARD:
        ...
        if settings.enable_time_limits:
            ...
            if (settings.min_time_seconds is not None and 
                estimated_duration < settings.min_time_seconds):
            ...
            elif (settings.max_time_seconds is not None and 
                  estimated_duration > settings.max_time_seconds):
        ...
        raise ValueError(f"Unknown video generation mode: {settings.video_generation_mode}")
...
def validate_video_generation_settings(settings: VideoGenerationSettings) -> None:
    ...
    if (settings.min_time_seconds is not None and 
        settings.max_time_seconds is not None and 
        settings.min_time_seconds >= settings.max_time_seconds):
    ...
    if (settings.video_generation_mode == VideoGenerationMode.TARGET and 
        settings.target_time_seconds is None):
        raise ValueError("Target time must be specified for target mode")
...
def get_effective_video_settings(camera_settings: dict, timelapse_settings: dict) -> VideoGenerationSettings:
    """
    ...
    def get_value(key: str, default_value=None):
        ...
        if timelapse_value is not None:
            return timelapse_value
        return camera_settings.get(key, default_value)
    ...
    return VideoGenerationSettings(
        ...
        min_time_seconds=get_value("min_time_seconds"),
        max_time_seconds=get_value("max_time_seconds"),
        target_time_seconds=get_value("target_time_seconds"),
        fps_bounds_min=get_value("fps_bounds_min", 1),
        fps_bounds_max=get_value("fps_bounds_max", 60)
    )


def preview_video_calculation(total_images: int, settings: VideoGenerationSettings) -> dict:
    """
