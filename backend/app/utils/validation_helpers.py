...
def validate_display_format(display: Optional[str]) -> Optional[str]:
    ...
    valid_displays = ["temp_only", "with_unit", "conditions_only", "temp_and_conditions"]
...
def validate_image_scale(scale: Optional[int], min_scale: int = 10, max_scale: int = 500) -> int:
    """
...
def validate_boolean_property(value: Optional[bool], property_name: str) -> Optional[bool]:
    """
...
def create_default_video_settings(video_name: Optional[str] = None, generation_type: str = "manual") -> dict:
    """
...
def create_health_response(health_model, success_message: str, degraded_message: str) -> tuple[dict, int]:
    """
...
def get_effective_automation_settings(timelapse_settings: dict, camera_settings: Optional[dict] = None) -> dict:
    """
...
def create_timelapse_automation_response(timelapse_id: int, effective_settings: Dict[str, Any], timelapse_model: Any) -> Dict[str, Any]:
    """
    ...
    automation_fields = [
        'video_automation_mode', 'video_generation_mode', 'target_fps', 'quality',
        'framerate', 'milestone_config', 'generation_schedule', 'enable_milestones',
        'milestone_intervals', 'milestone_thresholds'
    ]
    ...
    for field_name in automation_fields:
        ...
        value = effective_settings.get(field_name) or getattr(timelapse_model, field_name, None)
...
def validate_automation_mode_updates(update_data: dict) -> tuple[bool, Optional[str]]:
    ...
    if "video_automation_mode" in update_data:
        if update_data["video_automation_mode"] not in VIDEO_AUTOMATION_MODES_LIST:
            return False, f"Invalid automation mode. Must be one of: {', '.join(VIDEO_AUTOMATION_MODES_LIST)}"
    ...
    if "video_generation_mode" in update_data:
        if update_data["video_generation_mode"] not in VIDEO_GENERATION_MODES_LIST:
            return False, f"Invalid generation mode. Must be one of: {', '.join(VIDEO_GENERATION_MODES_LIST)}"
...
def create_default_timelapse_data(camera_id: int) -> dict:
    ...
    """
    from ..models.shared_models import VideoGenerationMode, VideoAutomationMode
...
def validate_camera_id_match(url_camera_id: int, body_camera_id: int) -> tuple[bool, Optional[str]]:
    """
...
def calculate_thumbnail_percentages(thumbnail_count: int, small_count: int, total_images: int) -> dict:
    """
