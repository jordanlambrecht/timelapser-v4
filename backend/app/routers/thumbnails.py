...
def generate_thumbnails_for_image(
    thumbnail_processor, image_path: str, image_id: int
) -> Dict[str, Any]:
...
async def regenerate_all_thumbnails_background():
    ...
    try:
        ...
        logger.info(
            f"Starting thumbnail regeneration for {regeneration_state['total']} images"
        )
        ...
        for i, image_row in enumerate(images_to_process):
            ...
            regeneration_state["current_image"] = (
                f"Camera {image['camera_id']} - Image {image['id']}"
            )
            ...
            result = generate_thumbnails_for_image(
                thumbnail_processor, image["file_path"], image["id"]
            )
            ...
            if result["success"]:
                ...
                logger.warning(
                    f"Failed to regenerate thumbnails for image {image['id']}: {result.get('error')}"
                )
        ...
        logger.info(
            f"Thumbnail regeneration completed: {regeneration_state['completed']}/{regeneration_state['total']} successful, {regeneration_state['errors']} errors"
        )
        ...
        regeneration_state["active"] = False
        sync_db.broadcast_event(
            {"type": "thumbnail_regeneration_error", "error": str(e)}
        )
...
async def start_thumbnail_regeneration(background_tasks: BackgroundTasks):
    ...
    if regeneration_state["active"]:
        raise HTTPException(
            status_code=409, detail="Thumbnail regeneration already in progress"
        )
    ...
    return {"message": "Thumbnail regeneration started", "status": "started"}
...
async def cancel_thumbnail_regeneration():
    ...
    if not regeneration_state["active"]:
        raise HTTPException(
            status_code=400, detail="No thumbnail regeneration in progress"
        )
    ...
    return {"message": "Thumbnail regeneration cancelled", "status": "cancelled"}
...
async def get_thumbnail_stats():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get thumbnail statistics"
        )
