...
Responsibilities: Timelapse lifecycle operations (start/pause/stop/complete), 
                 entity CRUD, progress tracking
Interactions: Uses TimelapseService for business logic, broadcasts SSE events 
...
async def create_timelapse(
    ...
    try:
        new_timelapse = await timelapse_service.create_new_timelapse(camera_id, timelapse_data)
        logger.info(f"Created timelapse {new_timelapse.id} for camera {camera_id} in timezone {timezone_str}")
...
async def update_timelapse(
    ...
    try:
        updated_timelapse = await timelapse_service.update_timelapse(timelapse_id, timelapse_data)
...
@router.post("/{timelapse_id}/start", response_model=Timelapse, status_code=status.HTTP_200_OK)
...
@router.post("/{timelapse_id}/pause", response_model=Timelapse, status_code=status.HTTP_200_OK)
...
@router.post("/{timelapse_id}/stop", response_model=Timelapse, status_code=status.HTTP_200_OK)
...
@router.post("/{timelapse_id}/complete", response_model=Timelapse, status_code=status.HTTP_200_OK)
