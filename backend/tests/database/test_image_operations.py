...
class MockImage:
    def __init__(self, **kwargs):
        ...
        self.updated_at = (
            kwargs.get("updated_at", datetime.now(timezone.utc))
            if "updated_at" not in kwargs or kwargs["updated_at"] is not None
            else None
        )
...
class TestImageQueryBuilders:
    ...
    def test_images_by_timelapse_query_pattern(self):
        ...
        mock_query = (
            "SELECT * FROM images WHERE timelapse_id = %s ORDER BY captured_at ASC"
        )
    ...
    def test_images_by_camera_query_pattern(self):
        ...
        mock_query = (
            "SELECT * FROM images WHERE camera_id = %s ORDER BY captured_at DESC"
        )
...
class TestImageCollectionETags:
    ...
    def test_generate_collection_etag_for_images(self):
        ...
        images = [
            MockImage(
                id=1, updated_at=datetime(2024, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
            ),
            MockImage(
                id=2, updated_at=datetime(2024, 1, 1, 13, 0, 0, tzinfo=timezone.utc)
            ),
            MockImage(
                id=3, updated_at=datetime(2024, 1, 1, 14, 0, 0, tzinfo=timezone.utc)
            ),
        ]
...
class TestImageOperationsCaching:
    ...
    def test_flagged_images_caching_benefit(self):
        ...
        def mock_get_flagged_images():
            nonlocal call_count
            call_count += 1
            return [MockImage(id=1, is_flagged=True), MockImage(id=2, is_flagged=True)]
...
class TestImageBatchOperations:
    ...
    def test_batch_image_retrieval_pattern(self):
        ...
        chunks = [
            image_ids[i : i + chunk_size] for i in range(0, len(image_ids), chunk_size)
        ]
...
def simulate_image_collection_etag(images: List[MockImage]) -> str:
    ...
    latest_time = max(img.updated_at or img.captured_at for img in images)
