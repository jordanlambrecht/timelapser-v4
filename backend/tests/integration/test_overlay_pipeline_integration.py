...
def mock_overlay_pipeline_services(mock_async_database, mock_sync_database):
    ...
    class MockOverlayServices:
        ...
        def create_mock_timelapse_overlay(
            self, timelapse_id=1, preset_id=1, enabled=True
        ):
        ...
        def create_mock_asset(
            self, name="Test Logo", file_path="/test/assets/logo.png"
        ):
        ...
        def create_mock_job(
            self, image_id=1, timelapse_id=1, preset_id=1, status=JobStatus.PENDING
        ):
            ...
            job = OverlayJob(
                ...
                completed_at=(
                    datetime.utcnow() if status == JobStatus.COMPLETED else None
                ),
                error_message=None,
...
def overlay_integration_pipeline(mock_overlay_pipeline_services):
    ...
    async_overlay_ops.get_preset_by_id.side_effect = (
        lambda preset_id: services.presets.get(preset_id)
    )
    async_overlay_ops.get_all_assets.return_value = list(services.assets.values())
    async_overlay_ops.get_asset_by_id.side_effect = (
        lambda asset_id: services.assets.get(asset_id)
    )
    async_overlay_ops.get_timelapse_overlay.side_effect = lambda timelapse_id: next(
        (
            config
            for config in services.timelapse_overlays.values()
...
class TestOverlayPipelineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_timelapse_overlay_configuration_flow(
        self, overlay_integration_pipeline
    ):
        ...
        created_config = services.create_mock_timelapse_overlay(
            timelapse_id=1, preset_id=preset.id
        )
        async_overlay_ops.create_timelapse_overlay.return_value = created_config

        # Test configuration creation
        result = await integration_service.create_or_update_timelapse_overlay_config(
            config_data
        )
    ...
    async def test_asset_management_integration(self, overlay_integration_pipeline):
        ...
        watermark_asset = services.create_mock_asset(
    ...
    async def test_job_queue_integration(self, overlay_integration_pipeline):
        ...
        processing_job = services.create_mock_job(
            image_id=2, status=JobStatus.PROCESSING
        )
        ...
        result = await async_job_ops.mark_job_completed(
            pending_job.id, "/data/overlays/output_1.jpg", processing_time_ms=2500
        )
        assert result is True
    ...
    @pytest.mark.asyncio
    async def test_overlay_configuration_validation_integration(
        self, overlay_integration_pipeline
    ):
    ...
    async def test_preview_generation_integration(self, overlay_integration_pipeline):
        ...
        with patch.object(
            integration_service, "_generate_preview_image"
        ) as mock_generate, patch.object(
            integration_service, "_capture_temp_image"
        ) as mock_capture:
            ...
            preview_result = await integration_service.generate_overlay_preview(
                preview_request
            )
    ...
    async def test_error_handling_across_pipeline(self, overlay_integration_pipeline):
        ...
        async_overlay_ops.get_all_presets.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_sync_async_coordination(
        self, overlay_integration_pipeline, mock_sync_database
    ):
        ...
        result = sync_job_service.overlay_job_ops.mark_job_completed(
            test_job.id, "/data/overlays/output.jpg", processing_time_ms=3000
        )
        assert result is True
    ...
    async def test_complete_overlay_workflow(self, overlay_integration_pipeline):
        ...
        async_overlay_ops.get_timelapse_overlay.return_value = (
        ...
        timelapse_config = (
            await integration_service.create_or_update_timelapse_overlay_config(
