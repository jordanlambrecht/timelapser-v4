...
class TestSchedulerTrustModelIntegration:
    ...
    def test_scheduling_service_validation_comprehensive(
        self, mock_sync_database, mock_settings_service
    ):
        ...
        with patch(
            "app.database.camera_operations.SyncCameraOperations"
        ) as MockCameraOps:
            with patch(
                "app.database.timelapse_operations.SyncTimelapseOperations"
            ) as MockTimelapseOps:
                ...
                with patch.object(
                    scheduling_service.settings_ops, "get_setting"
                ) as mock_get_setting:
    ...
    def test_scheduler_worker_uses_validation_results(
        self, mock_sync_database, mock_settings_service
    ):
        ...
        with patch.object(
            scheduling_service, "validate_capture_readiness"
        ) as mock_validate:
            # Test valid scenario
            ...
            with patch.object(
                scheduler_worker.timelapse_ops, "get_timelapse_by_id"
            ) as mock_get_timelapse:
                mock_timelapse = MagicMock()
                mock_timelapse.camera_id = 1
    ...
    def test_scheduler_worker_respects_validation_failures(
        self, mock_sync_database, mock_settings_service
    ):
    ...
    def test_capture_worker_trusts_scheduler_validation(
        self, mock_sync_database, mock_settings_service
    ):
        ...
        with patch.object(
            capture_worker.timelapse_ops, "get_timelapse_by_id"
        ) as mock_get_timelapse:
            mock_timelapse = MagicMock()
            mock_timelapse.camera_id = 1
            mock_get_timelapse.return_value = mock_timelapse
            ...
            with patch.object(
                capture_worker.camera_service, "get_camera_by_id"
            ) as mock_get_camera:
                mock_camera = MagicMock()
                mock_camera.id = 1
    ...
    def test_end_to_end_scheduler_trust_flow(
        self, mock_sync_database, mock_settings_service
    ):
    ...
    def test_validation_error_types_and_handling(
        self, mock_sync_database, mock_settings_service
    ):
    ...
    def test_scheduler_worker_dependency_injection(
        self, mock_sync_database, mock_settings_service
    ):
    ...
    def test_capture_worker_minimal_validation(
        self, mock_sync_database, mock_settings_service
    ):
...
class TestSchedulingServiceValidation:
    ...
    def test_scheduling_service_created_by_factory(
        self, mock_sync_database, mock_settings_service
    ):
        ...
        assert isinstance(
            workflow_orchestrator.scheduling_service, SyncSchedulingService
        )
