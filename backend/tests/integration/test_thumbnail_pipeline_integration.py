...
from app.services.scheduling.scheduler_authority_service import (
...
class TestThumbnailPipelineIntegration:
    ...
    def mock_databases(self):
        ...
        return {"sync": mock_sync_db, "async": mock_async_db}
    ...
    def thumbnail_pipeline(self, mock_databases):
        ...
        return ThumbnailPipeline(
            database=mock_databases["sync"], async_database=mock_databases["async"]
        )
    ...
    @pytest.mark.asyncio
    async def test_pipeline_scheduler_authority_integration(
        self, thumbnail_pipeline, mock_scheduler_authority, sample_image_path, temp_dir
    ):
        ...
        with patch.object(thumbnail_pipeline, "generate_thumbnail") as mock_generate:
            ...
            scheduler_result = (
                await mock_scheduler_authority.schedule_immediate_thumbnail_generation(
                    image_id=123, priority=SSEPriority.HIGH
                )
            )
            ...
            pipeline_result = thumbnail_pipeline.generate_thumbnail(
                source_path=sample_image_path, output_path=output_path
            )
    ...
    def test_pipeline_scheduler_trust_model(
        self, thumbnail_pipeline, sample_image_path, temp_dir
    ):
        ...
        with patch.object(
            thumbnail_pipeline.thumbnail_generator, "generate_thumbnail"
        ) as mock_gen:
    ...
    @pytest.mark.asyncio
    async def test_pipeline_job_queue_integration(
        self, thumbnail_pipeline, sample_thumbnail_job
    ):
        ...
        with patch.object(thumbnail_pipeline, "async_job_service") as mock_job_service:
            ...
            created_job = await mock_job_service.create_job(
                image_id=123, priority=JobPriority.HIGH
            )
    ...
    def test_complete_thumbnail_workflow(
        self, thumbnail_pipeline, sample_image_path, temp_dir
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                thumbnail_pipeline.small_generator, "generate_small_image"
            ) as mock_small:
    ...
    def test_batch_workflow_integration(self, thumbnail_pipeline, temp_dir):
        ...
        with patch.object(
            thumbnail_pipeline.batch_generator, "generate_thumbnails_batch"
        ) as mock_batch:
    ...
    def test_pipeline_error_handling_and_recovery(
        ...
        with patch.object(
            thumbnail_pipeline.thumbnail_generator,
            "generate_thumbnail",
    ...
    def test_scheduler_trust_model_minimal_validation(
        self, thumbnail_pipeline, sample_image_path, temp_dir
    ):
    ...
    @pytest.mark.asyncio
    async def test_scheduler_authority_delegation_pattern(
        self, mock_scheduler_authority
    ):
        ...
        authority_result = (
            await mock_scheduler_authority.schedule_immediate_thumbnail_generation(
                image_id=456, priority=SSEPriority.HIGH
            )
        )
