...
from app.services.thumbnail_pipeline.generators.batch_thumbnail_generator import BatchThumbnailGenerator
from app.services.thumbnail_pipeline.generators.thumbnail_generator import ThumbnailGenerator
from app.services.thumbnail_pipeline.generators.small_image_generator import SmallImageGenerator
...
class TestBatchThumbnailGenerator:
    ...
    def test_generate_thumbnails_batch_success(self, batch_generator, sample_images, output_dirs):
        """Test successful batch thumbnail generation."""
        # Mock the individual generators
        with patch.object(batch_generator.thumbnail_generator, 'generate_thumbnail') as mock_thumb:
    ...
    def test_generate_thumbnails_batch_partial_failure(self, batch_generator, sample_images, output_dirs):
        ...
        with patch.object(batch_generator.thumbnail_generator, 'generate_thumbnail', side_effect=mock_generate_side_effect):
            # Create batch job
            jobs = []
            for i, source in enumerate(sample_images):
                jobs.append({
    ...
    def test_generate_small_images_batch_success(self, batch_generator, sample_images, output_dirs):
        """Test successful batch small image generation."""
        # Mock the small image generator
        with patch.object(batch_generator.small_generator, 'generate_small_image') as mock_small:
    ...
    def test_generate_both_batch_success(self, batch_generator, sample_images, output_dirs):
        ...
        with patch.object(batch_generator.thumbnail_generator, 'generate_thumbnail') as mock_thumb:
            with patch.object(batch_generator.small_generator, 'generate_small_image') as mock_small:
                ...
                assert result.get('success') is True
                assert result.get('total_jobs') == len(sample_images)
                assert result.get('thumbnail_results', {}).get('successful_jobs') == len(sample_images)
                assert result.get('small_image_results', {}).get('successful_jobs') == len(sample_images)
    ...
    def test_batch_generation_with_progress_callback(self, batch_generator, sample_images, output_dirs):
        ...
        with patch.object(batch_generator.thumbnail_generator, 'generate_thumbnail') as mock_thumb:
            ...
            assert all(update['total'] == len(sample_images) for update in progress_updates)
    ...
    def test_concurrent_processing_limits(self, sample_images, output_dirs):
        ...
        with patch.object(batch_generator.thumbnail_generator, 'generate_thumbnail', side_effect=mock_generate_with_tracking):
            jobs = []
            for i, source in enumerate(sample_images):
                jobs.append({
                    'source_path': source,
    ...
    def test_batch_generation_exception_handling(self, batch_generator, sample_images, output_dirs):
        ...
        with patch.object(batch_generator.thumbnail_generator, 'generate_thumbnail') as mock_thumb:
            ...
            assert result.get('success') is True  # Batch continues despite individual failures
            assert result.get('total_jobs') == len(sample_images)
            assert result.get('failed_jobs') == len(sample_images)
            assert len(result.get('failures', [])) == len(sample_images)

    def test_batch_generation_mixed_results(self, batch_generator, sample_images, output_dirs):
        ...
        with patch.object(batch_generator.thumbnail_generator, 'generate_thumbnail', side_effect=mock_alternating_results):
            jobs = []
            for i, source in enumerate(sample_images):
                jobs.append({
                    'source_path': source,
    ...
    def test_batch_generation_scheduler_integration(self, batch_generator, sample_images, output_dirs):
