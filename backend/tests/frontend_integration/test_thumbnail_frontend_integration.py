...
class TestThumbnailFrontendIntegration:
        ...
        class MockFrontendClient:
            ...
            async def post(self, endpoint, json_data=None):
                self.requests.append(
                    {"method": "POST", "endpoint": endpoint, "data": json_data}
                )
    ...
    @pytest.mark.asyncio
    async def test_thumbnail_regeneration_modal_workflow(
        self, mock_frontend_client, mock_sse_client, mock_thumbnail_service_dependencies
    ):
        ...
        mock_frontend_client.set_response(
            "/api/thumbnails/regenerate-all/status",
            {
                "active": False,
                "progress": 0,
                "total": 0,
        ...
        status_response = await mock_frontend_client.get(
            "/api/thumbnails/regenerate-all/status"
        )
        ...
        bulk_request = BulkThumbnailRequest(
            image_ids=[1, 2, 3, 4, 5], priority=ThumbnailJobPriority.HIGH
        )
        ...
        mock_sse_client.add_event_handler(
            "thumbnail_regeneration_progress", progress_handler
        )

        # Simulate worker processing jobs and sending progress
        for i in range(1, 6):
            mock_sse_client.simulate_event(
        ...
        mock_sse_client.add_event_handler(
            "thumbnail_regeneration_complete", completion_handler
        )
        mock_sse_client.simulate_event(
            "thumbnail_regeneration_complete",
            {"completed": 5, "total": 5, "errors": 0, "processing_time_seconds": 12.5},
        )
    ...
    @pytest.mark.asyncio
    async def test_thumbnail_job_cancellation_workflow(
        self, mock_frontend_client, mock_sse_client, mock_thumbnail_service_dependencies
    ):
        ...
        bulk_request = BulkThumbnailRequest(
            image_ids=[1, 2, 3], priority=ThumbnailJobPriority.MEDIUM
        )
        ...
        mock_frontend_client.set_response(
            "/api/thumbnails/regenerate-all/cancel",
            {
                "success": True,
                "cancelled_jobs": 2,
        ...
        mock_sse_client.add_event_handler(
            "thumbnail_regeneration_cancelled", cancellation_handler
        )
        mock_sse_client.simulate_event(
            "thumbnail_regeneration_cancelled",
            {"cancelled_jobs": 2, "remaining_jobs": 1},
        )
    ...
    async def test_sse_event_filtering_and_handling(self, mock_sse_client):
        ...
        mock_sse_client.add_event_handler(
            "thumbnail_regeneration_progress", handle_progress
        )
        mock_sse_client.add_event_handler(
            "thumbnail_regeneration_complete", handle_complete
        )
        ...
        mock_sse_client.simulate_event(
            "thumbnail_job_created", {"job_id": 456, "image_id": 789}
        )
        ...
        mock_sse_client.simulate_event(
            ...
            {"error": "Failed to process image due to corruption", "image_id": 999},
        )
    ...
    async def test_api_endpoint_integration(
        self, mock_frontend_client, mock_thumbnail_service_dependencies
    ):
        ...
        job = await thumbnail_service.queue_single_image_thumbnail(
            image_id=123, priority=ThumbnailJobPriority.HIGH
        )
        ...
        mock_frontend_client.set_response(
            "/api/images/123/generate-thumbnail",
            {
                "success": True,
                "job_id": job.id,
        ...
        generate_response = await mock_frontend_client.post(
            "/api/images/123/generate-thumbnail"
        )
        ...
        bulk_request = BulkThumbnailRequest(
            image_ids=[100, 101, 102], priority=ThumbnailJobPriority.MEDIUM
        )
        ...
        bulk_api_response = await mock_frontend_client.post(
            "/api/thumbnails/generate-bulk",
            {"image_ids": [100, 101, 102], "priority": ThumbnailJobPriority.MEDIUM},
        )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_and_user_feedback(
        self, mock_sse_client, mock_thumbnail_service_dependencies
    ):
        ...
        # Check error details
        disk_space_error = next(
            e for e in error_events if "DISK_SPACE" in e["data"].get("error_code", "")
        )
        ...
        worker_crash_error = next(
            e for e in error_events if "WORKER_CRASH" in e["data"].get("error_code", "")
        )
    ...
    def test_frontend_component_state_management(self):
        ...
        # 4. Completion
        modal_state.handle_complete_event({"completed": 10, "total": 10})
        assert not modal_state.progress["active"]
