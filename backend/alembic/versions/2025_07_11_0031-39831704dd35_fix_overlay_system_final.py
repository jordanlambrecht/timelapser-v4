...
def upgrade() -> None:
    ...
    try:
        connection.execute(sa.text("DROP TABLE IF EXISTS overlay_generation_jobs CASCADE"))
        ...
        connection.execute(sa.text("ALTER TABLE images DROP COLUMN IF EXISTS overlay_path"))
        connection.execute(sa.text("ALTER TABLE images DROP COLUMN IF EXISTS has_valid_overlay"))
        connection.execute(sa.text("ALTER TABLE images DROP COLUMN IF EXISTS overlay_updated_at"))
    ...
    try:
        op.create_table(
            ...
            sa.Column('overlay_config', postgresql.JSONB(), nullable=False, server_default='{}'),
    ...
    try:
        op.create_table(
            ...
            sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('NOW()')),
            sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('NOW()')),
            sa.ForeignKeyConstraint(['timelapse_id'], ['timelapses.id'], ondelete='CASCADE'),
    ...
    try:
        op.create_table(
            ...
            sa.Column('job_type', sa.String(20), nullable=False, server_default='single'),
            sa.Column('retry_count', sa.Integer(), nullable=False, server_default='0'),
            sa.Column('error_message', sa.Text(), nullable=True),
            sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('NOW()')),
            sa.Column('started_at', sa.TIMESTAMP(timezone=True), nullable=True),
            sa.Column('completed_at', sa.TIMESTAMP(timezone=True), nullable=True),
            sa.CheckConstraint("priority IN ('low', 'medium', 'high')", name='ck_overlay_generation_jobs_priority'),
            sa.CheckConstraint("status IN ('pending', 'processing', 'completed', 'failed', 'cancelled')", name='ck_overlay_generation_jobs_status'),
            sa.CheckConstraint("job_type IN ('single', 'batch', 'priority')", name='ck_overlay_generation_jobs_job_type'),
            sa.CheckConstraint('retry_count >= 0 AND retry_count <= 5', name='ck_overlay_generation_jobs_retry_count'),
            sa.CheckConstraint(
    ...
    try:
        op.add_column('images', sa.Column('overlay_path', sa.Text(), nullable=True,
                                         comment='Path to generated overlay image relative to data directory'))
        print("✅ Added overlay_path column to images table")
        
        op.add_column('images', sa.Column('has_valid_overlay', sa.Boolean(), nullable=False, server_default='false',
                                         comment='Whether this image has a valid generated overlay'))
        print("✅ Added has_valid_overlay column to images table")
        
        op.add_column('images', sa.Column('overlay_updated_at', sa.TIMESTAMP(timezone=True), nullable=True,
    ...
    try:
        ...
        op.create_index('idx_overlay_presets_config_gin', 'overlay_presets', ['overlay_config'], postgresql_using='gin')
        print("✅ Created GIN index on overlay_presets.overlay_config")
        
        op.create_index('idx_timelapse_overlays_config_gin', 'timelapse_overlays', ['overlay_config'], postgresql_using='gin')
        print("✅ Created GIN index on timelapse_overlays.overlay_config")
        
        # Job queue indexes
        op.create_index('idx_overlay_generation_jobs_active', 'overlay_generation_jobs', 
                       ['priority', 'created_at'], 
        ...
        op.create_index('idx_overlay_generation_jobs_completed', 'overlay_generation_jobs', 
                       ['completed_at'],
        ...
        op.create_index('idx_timelapse_overlays_active', 'timelapse_overlays', 
                       ['timelapse_id'],
        ...
        # Asset indexes
        op.create_index('idx_overlay_assets_images', 'overlay_assets', 
                       ['mime_type'],
                       postgresql_where="mime_type LIKE 'image/%'")
        print("✅ Created partial index for image assets")
        
        # Built-in preset index
        op.create_index('idx_overlay_presets_builtin', 'overlay_presets', 
                       ['name'],
        ...
        op.create_index('idx_images_overlay_updated_at', 'images', ['overlay_updated_at'])
