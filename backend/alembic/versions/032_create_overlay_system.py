...
def upgrade() -> None:
    ...
    if not result.fetchone():
        op.add_column('images', sa.Column('overlay_path', sa.Text(), nullable=True, comment='Path to generated overlay image file'))
    ...
    """))
    if not result.fetchone():
        op.add_column('images', sa.Column('has_valid_overlay', sa.Boolean(), nullable=False, server_default='false', comment='Whether image has successfully generated overlay'))
    ...
    """))
    if not result.fetchone():
        op.add_column('images', sa.Column('overlay_updated_at', sa.TIMESTAMP(timezone=True), nullable=True, comment='Timestamp of last overlay generation'))
    ...
    try:
        op.create_index('idx_overlay_presets_config_gin', 'overlay_presets', ['overlay_config'], postgresql_using='gin', if_not_exists=True)
        print("✅ Created GIN index on overlay_presets.overlay_config")
    except Exception as e:
        print(f"⚠️ GIN index on overlay_presets.overlay_config skipped: {e}")

    try:
        op.create_index('idx_timelapse_overlays_config_gin', 'timelapse_overlays', ['overlay_config'], postgresql_using='gin', if_not_exists=True)
    ...
    try:
        op.create_index('idx_images_has_valid_overlay', 'images', ['has_valid_overlay'], if_not_exists=True)
        print("✅ Created index on images.has_valid_overlay")
    except Exception as e:
        print(f"⚠️ Images overlay validity index skipped: {e}")

    try:
        op.create_index('idx_images_overlay_updated_at', 'images', ['overlay_updated_at'], if_not_exists=True)
    ...
    try:
        ...
        for preset in builtin_presets:
            ...
            result = connection.execute(sa.text(
                "SELECT id FROM overlay_presets WHERE name = :name"
            ), {'name': preset['name']})
