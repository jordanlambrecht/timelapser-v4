...
def upgrade():
    ...
    try:
        ...
        op.add_column(
            "images", sa.Column("weather_temperature", sa.Float, nullable=True)
        )
        op.add_column(
            "images", sa.Column("weather_conditions", sa.String(255), nullable=True)
        )
        op.add_column("images", sa.Column("weather_icon", sa.String(50), nullable=True))
        op.add_column(
            "images",
            sa.Column("weather_fetched_at", sa.TIMESTAMP(timezone=True), nullable=True),
        )
        ...
        op.create_index(
            "idx_images_weather_fetched_at", "images", ["weather_fetched_at"]
        )
        op.create_index(
            "idx_images_weather_conditions", "images", ["weather_conditions"]
        )
