...
Revision ID: 005_video_gen_settings
Revises: 004_add_auto_stop_at
Create Date: 2025-06-17 15:40:00.000000
...
down_revision = "004_add_auto_stop_at"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    video_generation_mode_enum = postgresql.ENUM(
        "standard", "target", name="video_generation_mode"
    )
    ...
    op.add_column(
        "cameras",
        sa.Column("video_generation_mode", video_generation_mode_enum, nullable=True),
    )
    op.add_column("cameras", sa.Column("standard_fps", sa.Integer(), nullable=True))
    op.add_column(
        "cameras", sa.Column("enable_time_limits", sa.Boolean(), nullable=True)
    )
    ...
    op.add_column(
        "timelapses", sa.Column("enable_time_limits", sa.Boolean(), nullable=True)
    )
    op.add_column(
        "timelapses", sa.Column("min_time_seconds", sa.Integer(), nullable=True)
    )
    op.add_column(
        "timelapses", sa.Column("max_time_seconds", sa.Integer(), nullable=True)
    )
    op.add_column(
        "timelapses", sa.Column("target_time_seconds", sa.Integer(), nullable=True)
    )
    op.add_column(
        "timelapses", sa.Column("fps_bounds_min", sa.Integer(), nullable=True)
    )
    op.add_column(
        "timelapses", sa.Column("fps_bounds_max", sa.Integer(), nullable=True)
    )

    # Add calculation metadata to videos table
    op.add_column(
        "videos",
        sa.Column("calculated_fps", sa.Numeric(precision=6, scale=2), nullable=True),
    )
    op.add_column("videos", sa.Column("target_duration", sa.Integer(), nullable=True))
    op.add_column(
        "videos",
        sa.Column("actual_duration", sa.Numeric(precision=8, scale=2), nullable=True),
    )
    op.add_column(
        "videos",
        sa.Column("fps_was_adjusted", sa.Boolean(), default=False, nullable=True),
    )
    ...
    op.execute(
        "UPDATE videos SET fps_was_adjusted = false WHERE fps_was_adjusted IS NULL"
    )
